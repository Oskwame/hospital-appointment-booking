...
export function AppointmentForm({ appointment, onClose, onCreated }: AppointmentFormProps) {
  const [services, setServices] = useState<Array<{ id: number; name: string; description: string; availableDates: string[]; timeSlots: string[] }>>([])
  ...
  const canSubmit = Boolean(name && email && serviceId && date && time)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${base}/appointments`, {
        ...
        credentials: "include",
        body: JSON.stringify({ name, email, gender, description, serviceId, date, time }),
      })
  ...
  <Card className="p-6 bg-white shadow-md rounded-xl max-w-5xl mx-auto">
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-600 mb-1">Available Time</label>
          <Select value={time || undefined} onValueChange={(v) => setTime(v)}>
            <SelectTrigger className="w-full rounded-lg border border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition">
