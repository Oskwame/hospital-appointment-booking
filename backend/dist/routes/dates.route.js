...
router.post('/', async (req, res) => {
    try {
        ...
        const created = await prismaClient_1.default.availableDate.create({ data: { appointmentDate: date } });
        res.status(201).json({
            id: created.id,
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const updated = await prismaClient_1.default.availableDate.update({ where: { id }, data: { isAvailable: is_available } });
        res.json({
            id: updated.id,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        if (!Number.isInteger(id))
            return res.status(400).json({ message: 'Invalid id' });
        await prismaClient_1.default.availableDate.delete({ where: { id } });
